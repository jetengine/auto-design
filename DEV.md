...
首次（7 步）：**7/7 通过，合计 23.4s**。加入测量族后（8 步）：

```
  ✅ PASS       0.4s  connect   COM 链路 / 版本 / 会话
  ✅ PASS       3.9s  health    超时熔断 + 阻塞自诊断 + 重建恢复
  ✅ PASS       2.7s  box       加料特征（Sketch + Pad）
  ✅ PASS       1.6s  pocket    去料特征（偏移平面 + Pocket）
  ✅ PASS       2.2s  fillet    几何引用（边拾取 + EdgeFillet）
  ✅ PASS       2.6s  measure   只读测量（体积/面积/重心 三独立证据）
  ✅ PASS       4.4s  export    交付（STEP，不可用则降级 IGES）
  ✅ PASS       6.6s  mcp       MCP 端到端（起服务器 + 调工具）

  8/8 通过，合计 24.3s
```

**24 秒**——这个数字比「8/8 通过」更重要。回归的执行率由它决定：24 秒的回归每次改动都会跑，5 分钟的只在提交前跑，20 分钟的没人跑。

而且注意：**多了一个完整能力，总耗时只涨了 0.9s**。这不是巧合，是因为每个脚本都只做「建最小模型 + 精确解比对」，没有一个在做压力测试。这个克制要一直保持——回归一旦变慢，它就开始被跳过，然后就等于不存在。

基线里几个值得记的实测点：

- `health` 那 3.9s 里有 3.0s 是**故意等的**（人为制造卡死后验证熔断），不是性能问题。
