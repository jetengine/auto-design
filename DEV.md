...
Chamfer 大概率一次过（同一条路）。Shell 次之。**Draft 我不指望首轮通过**——它要用 `part.CreateReferences()` 构造 `References` 集合，这是本项目第一次碰这个对象，加上两个枚举都不确定。

所以那几处的报错都写成了"说清是哪一步没过"，而不是甩个 `AttributeError`。首轮跑完把 `target_errors` 贴回来，就能据此收敛。
